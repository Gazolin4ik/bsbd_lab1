...
-- TRIGGER 1: UNIQUE TRACKING NUMBER
...
-- TRIGGER 4: AUTO TIMESTAMPS ON INSERT
...
-- TRIGGER (AUX): TOUCH updated_at ON UPDATE
...
-- TRIGGER 2: PROPAGATE SERVICE BASE_PRICE TO NEW SHIPMENTS
...
-- TRIGGER 3: BLOCK MASS DELETE + AUDIT
...
-- TRIGGER 5: PASSWORD CHANGE PROTECTION
...
INSERT INTO app.lab2_shipments_demo (
    ...
    shipment_type_id, status, weight, declared_value,
    price_original, price_final, created_at, updated_at
)
SELECT
    'LAB2-DEMO-TRK-UNIQ',
    1, 2, 1, 2,
    st.id, 'NEW', 0.5, NULL,
    st.base_price, st.base_price, NULL, NULL
...
BEGIN
    BEGIN
        ...
        VALUES (
            'LAB2-DEMO-TRK-UNIQ', 1, 2, 1, 2,
            (SELECT id FROM ref.shipment_types WHERE code='ECONOMY'),
            'NEW', 0.5, NULL,
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Expected error (unique tracking): %', SQLERRM;
    END;
END;
$$;

-- DEMO: TRIGGER 2
SELECT s.id, s.tracking_number, t.code, s.price_original, s.price_final
FROM app.lab2_shipments_demo s
...
-- DEMO: TRIGGER 3
TRUNCATE TABLE audit.lab2_delete_audit;
DELETE FROM app.lab2_shipments_demo WHERE tracking_number LIKE 'LAB2-DEMO-TRK-DEL-%';
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Expected error (mass delete): %', SQLERRM;
    END;
END;
$$;

-- DEMO: TRIGGER 4
DELETE FROM app.lab2_shipments_demo WHERE tracking_number = 'LAB2-DEMO-TRK-TS';
INSERT INTO app.lab2_shipments_demo (
    tracking_number, from_office_id, to_office_id, sender_id, recipient_id,
    shipment_type_id, status, weight, declared_value,
    price_original, price_final, created_at, updated_at
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Expected error (password blocked): %', SQLERRM;
    END;
END;
$$;
...
UPDATE app.employee_credentials
SET password_hash = md5(CURRENT_TIMESTAMP::text)
WHERE login = 'anna_ivanova';

SELECT db_username, can_change
FROM app.password_change_permissions
WHERE db_username = current_user;
