-- =============================================
-- ЛАБОРАТОРНАЯ РАБОТА №5: МАРКЕТИНГОВЫЕ МЕТРИКИ
-- Запуск из хоста:
--   ./run_tests.sh lab5
-- или напрямую:
...
\echo '=========================================='
\echo 'LAB5: ALL-TIME METRICS (LTV, AOV)'
\echo '=========================================='

-- 1. LTV (Lifetime Value) по каждому клиенту (sender)
WITH user_orders AS (
    SELECT
...
\echo ''
\echo '------------------------------------------'
\echo 'LAB5: TOP-5 CLIENTS BY AOV'
\echo '------------------------------------------'

-- 2. AOV (Average Order Value) и TOP-5 клиентов
WITH user_aov AS (
    SELECT
        s.sender_id                AS user_id,
...
\echo ''
\echo '=========================================='
\echo 'LAB5: REPORTING PERIOD METRICS (LAST MONTH)'
\echo '=========================================='
...
\echo ''
\echo '------------------------------------------'
\echo 'LAB5: ARPPU (LAST MONTH, PARTITIONED TABLE)'
\echo '------------------------------------------'
...
\echo ''
\echo '------------------------------------------'
\echo 'LAB5: TOP-3 MOST POPULAR SHIPMENT TYPES (LAST MONTH)'
\echo '------------------------------------------'

-- 5. TOP-3 популярных типов отправлений за последний месяц
...
\echo ''
\echo '------------------------------------------'
\echo 'LAB5: TOP-3 LEAST POPULAR SHIPMENT TYPES (LAST MONTH)'
\echo '------------------------------------------'

-- 6. TOP-3 наименее популярных типов (среди тех, что покупались)
WITH shipments_last_month AS (
    SELECT *
    FROM app.shipments_partitioned s
    WHERE s.created_at >= date_trunc('month', CURRENT_DATE) - interval '1 month'
